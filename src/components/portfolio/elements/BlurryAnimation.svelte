<script lang="ts">
  ...
  const maxMagicNum = total / 2;
  const magicNum = index > maxMagicNum ? total - index : index - 1;
  ...
  const screenToSVG = ({
    ...
    const svg = svgRef;
    let p = (svg as any).createSVGPoint();
    p.x = screenX;
    p.y = screenY;
    return p.matrixTransform((svg as any).getScreenCTM().inverse());
  };

  let coords = spring(
    { x: magicNum * 200 + 1200, y: magicNum * 100 },
    {
      stiffness: 0.05,
      damping: 0.7,
    }
...
<div
  class="w-full h-5 relative overflow-hidden"
  on:mousemove={(e) => {
    coords.set(screenToSVG({ x: e.clientX, y: e.clientY }));
  }}
  role="img"
  aria-label="Hero animation strip"
>
  <svg
    bind:this={svgRef}
    ...
    class="min-h-full min-w-full absolute inset-0 -scale-x-100 -z-20 pointer-events-none"
  >
